...
export const CandidateForm: React.FC<CandidateFormProps> = ({ onSubmit }) => {
  ...
  const [formType, setFormType] = useState<"nameContext" | "url">(
    "nameContext"
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (
      (formType === "nameContext" && name.trim() && context.trim()) ||
      (formType === "url" && url.trim())
    ) {
      setIsLoading(true);
      try {
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="w-full flex items-center justify-center px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-purple-500 hover:text-purple-500 transition-colors mb-4"
      >
              ...
              >
                ...
                <div className="group relative inline-block">
                  ...
                  <div className="absolute hidden group-hover:block w-64 px-4 py-2 bg-gray-800 text-white text-sm rounded-lg -right-64 top-6">
                    Enter some basic information about the candidate to aid in
                    our search. Info copy/pasted from their LinkedIn works
                    great.
                  </div>
                </div>
              </label>
